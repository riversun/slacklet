...
class SlackWebSocketSessionImpl extends AbstractSlackSessionImpl implements SlackSession, MessageHandler.Whole<String> {
  ...
  @Override
  public SlackMessageHandle<SlackMessageReply> sendPresenceSubMessageOverWebSocket(String[] ids) {
    SlackMessageHandleImpl<SlackMessageReply> handle = new SlackMessageHandleImpl<>(getNextMessageId());
    try {
  ...
  @Override
  public SlackMessageHandle<SlackMessageReply> sendRawMessageOverWebSocket(JsonObject messageJSON) {
    SlackMessageHandleImpl<SlackMessageReply> handle = new SlackMessageHandleImpl<>(getNextMessageId());
    try {
      websocketSession.getBasicRemote().sendText(messageJSON.toString());
    } catch (Exception e) {
  ...
  public void onMessage(String message) {
    ...
    if ("pong".equals(object.get("type").getAsString())) {
      ...
      Collection<SlackUser> slackUsers = getUsers();
